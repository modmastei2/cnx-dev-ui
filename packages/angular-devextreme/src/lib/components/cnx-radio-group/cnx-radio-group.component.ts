...
export class CnxRadioGroupComponent implements OnInit, OnChanges {
  ...
  get displayExpr(): string { return this.displayExprOption; }
  ...
  get valueExpr(): string { return this.valueExprOption; }
  public valueExprOption: string = 'value';

  @Input('radioGroupKey') public radioGroupKey: RadioGroupKey | null | undefined = null;
  @Input('cascadeBy') public cascadeBy: any;
  @Input('customDataSource') public customDataSource?: RadioGroupViewModel[] | any[];
  @Input('ignoreValue') public ignoreValue: string[] = [];

  @Input('dataSource')
  public dataSource: RadioGroupViewModel[] | any[] = [];
  ...
  constructor(
    @Optional() @Inject(RADIO_GROUP_DATA_PROVIDER) private service: RadioGroupDataProvider,
  ...
  private async setupDataSource(): Promise<void> {
    ...
    if (this.radioGroupKey && this.service) {
      const result = await lastValueFrom(
        this.service.getService(this.radioGroupKey, { cascadeBy: this.cascadeBy } as RadioGroupParam)
      );
    ...
    // 3. Static dataSource (passed via [dataSource] input)
    this.applyAutoDefault();
  }

  private applyIgnoreValue(items: RadioGroupViewModel[] | any[]): any[] {
    if (!this.ignoreValue?.length) return items;
    return items.filter((item) => !this.ignoreValue.includes(item[this.valueExprOption]));
  }

  private applyAutoDefault(): void {
    if (this.autoDefault && !this._value && this.dataSource.length > 0) {
      this._value = this.dataSource[0]?.[this.valueExprOption] ?? '';
      this.cdr.detectChanges();
    }
  }
}
