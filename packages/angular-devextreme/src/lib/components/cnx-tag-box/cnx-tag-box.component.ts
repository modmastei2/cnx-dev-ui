...
export class CnxTagBoxComponent implements OnInit, OnChanges {
    constructor(
        @Optional()
        @Inject(TAGBOX_DATA_PROVIDER)
        private service: TagBoxDataProvider,
    ...
    set searchExpr(val: string | null) {
        let _val = val || '';
        _val = _val.toString();
        if (!!_val && _val != this.searchExprOption)
            this.searchExprOption = _val;
    }
    ...
    set dropdownExpr(val: string | null) {
        let _val = val || '';
        _val = _val.toString();
        if (!!_val && _val != this.dropdownExprOption)
            this.dropdownExprOption = _val; // fixed old bug mapping wrong prop
    }
    ...
    @Input('showSelectionControls') public showSelectionControls: boolean =
        true;
    ...
    private async setupDataSourceOnLoad(
        ...
        // 2. Key-based service
        if (this.tagBoxKey && this.service) {
    ...
    private async setupDataSourceByKey(
